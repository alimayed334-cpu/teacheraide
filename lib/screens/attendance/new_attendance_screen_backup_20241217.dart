...
class _NewAttendanceScreenState extends State<NewAttendanceScreen> with WidgetsBindingObserver {
  ...
  Future<void> _loadAllAttendanceStatuses() async {
    ...
    for (final student in Provider.of<StudentProvider>(context, listen: false).students) {
      try {
  ...
  Future<void> _checkAtRiskStudents() async {
    ...
    final bool featureEnabled = prefs.getBool('at_risk_feature_enabled') ?? true;
    final bool averageCriteriaEnabled = prefs.getBool('at_risk_average_enabled') ?? true;
    final bool missedExamsCriteriaEnabled = prefs.getBool('at_risk_missed_exams_enabled') ?? true;
    final bool missedLecturesCriteriaEnabled = prefs.getBool('at_risk_missed_lectures_enabled') ?? true;
    ...
    if (!averageCriteriaEnabled && !missedExamsCriteriaEnabled && !missedLecturesCriteriaEnabled) {
      for (final student in Provider.of<StudentProvider>(context, listen: false).students) {
        atRisk[student.id!] = false;
      }
      setState(() {
    ...
    for (final student in Provider.of<StudentProvider>(context, listen: false).students) {
      try {
        ...
        if (missedLecturesCriteriaEnabled && missedLectures >= minMissedLectures) {
          isAtRisk = true;
        }
        
        // معيار المعدل المنخفض
        if (averageCriteriaEnabled && averagePercentage < minAveragePercentage) {
          isAtRisk = true;
        }
  ...
  void _saveComment(int studentId, String dateKey, String comment) async {
    ...
    try {
      ...
      for (var attendance in attendances) {
        final attendanceDateKey = DateFormat('yyyy-MM-dd').format(attendance.date);
        if (dateKey.contains(attendanceDateKey)) {
          await dbHelper.updateAttendance(attendance.copyWith(notes: comment));
          print('Updated attendance in database: ${attendance.notes} -> $comment');
          ...
          try {
            ...
            await prefs.setString(commentsKey, allComments.entries.map((e) => '${e.key}:${e.value}').join(','));
    ...
    try {
        ...
        for (final attendance in attendances) {
          if (attendance.notes != null && attendance.notes!.isNotEmpty) {
            ...
            setState(() {
              ...
              _studentComments[student.id!]!['${attendance.lectureId}_$dateKey'] = attendance.notes!;
            });
          }
        }
      }
      ...
      try {
        ...
        if (commentsString != null) {
          final Map<String, String> allComments = Map<String, String>.fromEntries(
  ...
  Future<void> _updateAttendanceDates(LectureModel oldLecture, LectureModel newLecture) async {
    try {
      // الحصول على جميع سجلات الحضور للمحاضرة القديمة
      final attendances = await _dbHelper.getAttendancesByLectureId(oldLecture.id!);
      ...
      for (final student in Provider.of<StudentProvider>(context, listen: false).students) {
        if (_attendanceStatus.containsKey(student.id!)) {
          final oldLectureKey = '${oldLecture.id}_${DateFormat('yyyy-MM-dd').format(oldLecture.date)}';
          final newLectureKey = '${newLecture.id}_${DateFormat('yyyy-MM-dd').format(newLecture.date)}';
          
          if (_attendanceStatus[student.id!]!.containsKey(oldLectureKey)) {
            _attendanceStatus[student.id!]![newLectureKey] = _attendanceStatus[student.id!]![oldLectureKey]!;
          ...
          if (_studentComments.containsKey(student.id!)) {
            if (_studentComments[student.id!]!.containsKey(oldLectureKey)) {
              _studentComments[student.id!]![newLectureKey] = _studentComments[student.id!]![oldLectureKey]!;
  ...
  Widget _buildAttendanceBox(StudentModel student, LectureModel lecture, AttendanceStatus currentStatus) {
    ...
    final AttendanceStatus status = _attendanceStatus[student.id!]?[lectureKey] ?? currentStatus;
                  ...
                  const PopupMenuItem(
                    ...
                    child: Row(
                      children: [
                        ...
                        SizedBox(width: 8),
                        Text('أبجدي', style: const TextStyle()),
                      ],
                    ),
                  ),
                  const PopupMenuItem(
                    ...
                    child: Row(
                      children: [
                        ...
                        SizedBox(width: 8),
                        Text('نسبة الحضور', style: const TextStyle()),
                      ],
                    ),
                  ),
                  const PopupMenuItem(
                    ...
                    child: Row(
                      children: [
                        ...
                        SizedBox(width: 8),
                        Text('نسبة الغياب', style: const TextStyle()),
                      ],
                    ),
                  ),
                  const PopupMenuItem(
                    ...
                    child: Row(
                      children: [
                        Icon(Icons.wc),
                        SizedBox(width: 8),
                        Text('الجنس', style: const TextStyle()),
                      ],
                    ),
                  ),
                ],
                          ...
                          return Container(
                            ...
                            child: Row(
                              children: [
                                ...
                                Container(
                                  width: 200, // زيادة العرض لإظهار الاسم بشكل أفضل
                                            ...
                                            child: student.photo == null 
                                                ? Text(
                                                    ...
                                                    style: const TextStyle(
                                                      color: Colors.black,
                                                      fontWeight: FontWeight.bold,
                                                      fontSize: 18, // زيادة حجم الحرف الأول
                                                    ),
                                                  ) 
                                                : null,
                                          ),
                                          // نقطة حمراء مصغرة للطلاب في خطر - أعلى الاسم فوق الدائرة
                                                  ...
                                                  children: [
                                                    Text(
                                                      ...
                                                      style: const TextStyle(
                                                        color: Colors.white,
                                                        fontWeight: FontWeight.w500,
                                                        fontSize: 14, // تقليل حجم الخط ليطابق الامتحانات
                                                        fontFamily: '', // إضافة خط عربي
                                                      ),
                                                      maxLines: 1,
                                                      overflow: TextOverflow.ellipsis,
                                ...
                                child: SingleChildScrollView(
                                  ...
                                  child: Row(
                                    ...
                                    children: _lectures.asMap().entries.map((entry) {
                                      ...
                                      final status = _attendanceStatus[student.id!]?[lectureKey] ??
  ...
  Future<void> _updateAttendance(int studentId, String lectureKey, AttendanceStatus status) async {
    if (!mounted) return;
  ...
  Future<bool> _isAnyRiskCriteriaEnabled() async {
    final prefs = await SharedPreferences.getInstance();
    final featureEnabled = prefs.getBool('unified_student_status_at_risk_feature_enabled') ?? true;
    if (!featureEnabled) return false;
    
    final averageEnabled = prefs.getBool('unified_student_status_at_risk_average_enabled') ?? true;
    final missedExamsEnabled = prefs.getBool('unified_student_status_at_risk_missed_exams_enabled') ?? true;
    final missedLecturesEnabled = prefs.getBool('unified_student_status_at_risk_missed_lectures_enabled') ?? true;
