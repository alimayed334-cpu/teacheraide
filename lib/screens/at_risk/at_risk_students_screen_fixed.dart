...
class _AtRiskStudentsScreenState extends State<AtRiskStudentsScreen> {
  ...
  int _minMissedLectures = 5;
  double _minAveragePercentage = 60.0;
  ...
  Future<void> _loadCriteria() async {
    ...
    setState(() {
      ...
      _minMissedExams = prefs.getInt('at_risk_min_missed_exams') ?? 2;
      _minMissedLectures = prefs.getInt('at_risk_min_missed_lectures') ?? 5;
      _minAveragePercentage = prefs.getDouble('at_risk_min_average') ?? 60.0;
    });
  }
  ...
  Future<void> _loadData() async {
    try {
      ...
      setState(() {
        _allClasses = classes;
        _selectedClassIds = classes.map((c) => c.id!).toList();
        _pdfSelectedClassIds = classes.map((c) => c.id!).toList();
      });
  ...
  Future<void> _calculateAtRiskStudents() async {
    ...
    for (final classModel in _allClasses) {
      ...
      for (final student in students) {
        ...
        final latestGrades = <String, Grade>{};
            ...
            return pw.Directionality(
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  pw.Text(
                    'التاريخ / Date: ${dateFormat.format(now)}',
                    style: pw.TextStyle(
                      font: arabicFont,
                      fontSize: 14,
              ...
              Expanded(
                child: GestureDetector(
                  ...
                  child: Container(
                    padding: const EdgeInsets.all(12),
                    decoration: BoxDecoration(
                      color: _showAllClasses ? Colors.red.withOpacity(0.2) : Colors.transparent,
                      borderRadius: BorderRadius.circular(8),
