...
class _StudentAssignmentsScreenState extends State<StudentAssignmentsScreen> {
    ...
    for (final exam in filteredExams) {
      ...
      if (grade != null && _shouldCountInAverage(grade.status ?? 'حاضر')) {
        ...
        if (percentage < 50) {
          ...
          distribution['85% فما فوق'] = (distribution['85% فما فوق'] ?? 0) + 1;
        }
      }
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xFF2A2A2A),
        title: Text('الواجبات / الامتحانات: ${widget.student.name}'),
        actions: [
          IconButton(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            overallGrade > 0 ? '${overallGrade.toStringAsFixed(1)}%' : 'غير متوفر',
            style: TextStyle(
              fontSize: 24,
              fontWeight: FontWeight.bold,
                  ...
                  Expanded(
                    child: LinearProgressIndicator(
                      ...
                      valueColor: AlwaysStoppedAnimation<Color>(
                        entry.key == '85% فما فوق' ? Colors.green :
                        entry.key == '75-85%' ? Colors.blue :
                        entry.key == '60-75%' ? Colors.orange : 
                        entry.key == '50-60%' ? Colors.deepOrange : Colors.red,
                      ),
                    ),
                  ),
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              if (grade != null && grade.comment != null && grade.comment!.isNotEmpty)
                Container(
                  ...
                  child: Row(
                    children: [
                      const Icon(Icons.comment, color: Color(0xFFFFD700), size: 16),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text(
                          grade.comment!,
